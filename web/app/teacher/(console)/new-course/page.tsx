...
export default function NewCoursePage() {
  ...
  const adopt = () => {
    ...
    setMsgs((m) => [...m, { from: 'agent', text: '已经帮你填到右侧啦，你可以再修改。确认后点「保存并开课」就能让孩子们进入课堂了。' }]);
    setPending(null);
  };
            ...
            {msgs.map((m, i) =>
              m.from === 'agent' ? (
                <div key={i} className="flex items-start gap-2.5 animate-rise">
                  ...
                  <div className="max-w-[85%] rounded-2xl rounded-tl-md border border-brand-soft bg-brand-faint px-4 py-3 text-sm leading-6.5 text-ink" style={{ lineHeight: 1.7 }}>
                    {m.text}
                  </div>
                </div>
              ) : (
                <div key={i} className="flex items-start justify-end gap-2.5 animate-rise">
                  <div className="max-w-[85%] rounded-2xl rounded-tr-md bg-brand px-4 py-3 text-sm text-white" style={{ lineHeight: 1.7 }}>
                    {m.text}
                  </div>
              ...
              <div className="flex items-start gap-2.5 animate-rise">
                ...
                <div className="rounded-2xl rounded-tl-md border border-brand-soft bg-brand-faint px-5 py-3">
                  <span className="flex gap-1.5">
                    {[0, 1, 2].map((i) => (
                      <span key={i} className="h-1.5 w-1.5 rounded-full bg-brand animate-blink" style={{ animationDelay: `${i * 0.25}s` }} />
                    ))}
                  </span>
                </div>
              </div>
        ...
        <section className="flex flex-col gap-5">
          ...
          {saved ? (
            <div className="card flex flex-col items-center gap-3 border-mint/40 bg-mint-soft/50 p-6 text-center animate-pop">
              ...
              <Link href="/onboarding/identity" className="btn-brand !px-5 !py-2.5 text-sm">
            ...
            <button
              onClick={() => subject.trim() && goal.trim() && setSaved(true)}
