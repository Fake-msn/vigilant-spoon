...
import { chatScript, regions, students, type ChatMsg } from '@/lib/data';
import { Icon } from '@/components/Icon';
import { PixelArt } from '@/components/art/PixelArt';
import { cakeMap, cakePalette, petMap, petPalette } from '@/components/art/pixelData';
  ...
  return (
    <div className="flex h-8 items-center justify-center gap-1.5" aria-hidden>
      {[0, 1, 2, 3, 4, 5, 6].map((i) => (
        <span
          key={i}
          className={`w-1.5 rounded-full ${active ? 'animate-wave' : ''} ${
...
function ChatInner() {
  ...
  const [visible, setVisible] = useState<ChatMsg[]>([chatScript[0]]);
  ...
  const done = visible.length >= chatScript.length;
  ...
  const talk = () => {
    ...
    setPhase('listening');
    const batch: ChatMsg[] = [];
    if (chatScript[nextIdx]?.from === 'me') batch.push(chatScript[nextIdx]);
    if (chatScript[nextIdx + batch.length]?.from === 'ai') batch.push(chatScript[nextIdx + batch.length]);

    timers.current.push(
      setTimeout(() => {
        if (batch[0]?.from === 'me') setVisible((v) => [...v, batch[0]]);
        setPhase('thinking');
      }, 1500),
      setTimeout(() => {
        ...
        if (ai) {
          setVisible((v) => [...v, ai]);
          if ('image' in ai && ai.image === 'cake') setDream('cake');
          if ('image' in ai && ai.image === 'dream') setDream('dream');
        }
        setPhase('idle');
      }, 2900)
    );
        ...
        <div ref={scrollRef} className="flex-1 space-y-5 overflow-y-auto px-6 py-6" aria-live="polite">
            ...
            m.from === 'ai' ? (
              <div key={i} className="flex items-start gap-3 animate-rise">
                ...
                <div className="max-w-[80%]">
                  <div className="rounded-2xl rounded-tl-md bg-brand-faint px-4.5 py-3.5 text-[15px] leading-7 text-ink border border-brand-soft" style={{ padding: '12px 18px' }}>
                    {m.text}
                  </div>
                  {'image' in m && m.image && (
              ...
              <div key={i} className="flex items-start justify-end gap-3 animate-rise">
                <div className="max-w-[80%] rounded-2xl rounded-tr-md bg-brand px-4 py-3 text-[15px] leading-7 text-white shadow-btn" style={{ padding: '12px 18px' }}>
                  {m.text}
                </div>
                <span className="h-10 w-10 shrink-0 overflow-hidden rounded-full border-2 border-white shadow-card">
                  <KidAvatar v={avatar} size={40} />
                </span>
              </div>
            )
          )}
          ...
          {phase === 'listening' && (
            <div className="flex items-start justify-end gap-3 animate-rise">
              <div className="rounded-2xl rounded-tr-md bg-brand/80 px-5 py-3 text-sm text-white">
                正在聆听…
              </div>
            </div>
          )}
            ...
            {!done ? (
              <>
                ...
                <p className="text-sm font-medium text-ink-soft">
                  {phase === 'listening' ? '我在听，慢慢说…' : phase === 'thinking' ? '小信正在想…' : '点击麦克风，告诉小信你的梦想吧'}
                </p>
              </>
            ) : (
              <div className="flex w-full flex-col items-center gap-3 animate-pop">
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  ...
                  >
                    ...
                    {saved ? '已存入成长档案 ✓' : '保存电子宠物'}
                  </button>
                  <button onClick={reset} className="btn-line !px-5 !py-2.5 text-sm">
                    <Icon name="refresh" size={16} />
                    再聊一次
                  </button>
                  <Link href="/profile" className="btn-line !px-5 !py-2.5 text-sm !border-brand !text-brand">
      ...
      <aside className="flex flex-col gap-5">
        ...
        <div className="card flex items-center gap-4 p-5">
          <span className="overflow-hidden rounded-lg" style={{ width: 64, height: 64 }}>
            <KidAvatar v={avatar} size={64} />
          </span>
          <div className="flex-1">
            <p className="text-lg font-bold text-ink">{student.name}</p>
            <p className="mt-0.5 text-xs font-medium text-ink-faint">{student.grade} · {region.name}</p>
          </div>
        ...
        <div className="card flex flex-col items-center gap-3 p-6 text-center">
          ...
          {dream === 'none' && (
            <>
              <span className="animate-floaty">
                <PixelArt map={petMap} palette={petPalette} size={110} title="小信伙伴" />
              </span>
          ...
          {dream === 'dream' && (
            <>
              ...
              <p className="tag bg-grape-soft text-grape">{student.dream ?? '蛋糕师'} · {student.name}</p>
            </>
          )}
        </div>
        ...
        <div
          className={`rounded-[10px] p-5 text-white shadow-card transition-colors ${
            done ? 'bg-gradient-to-br from-mint to-emerald-600' : 'bg-gradient-to-br from-brand to-brand-deep'
          }`}
        >
